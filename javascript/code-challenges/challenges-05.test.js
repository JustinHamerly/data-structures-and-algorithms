...
const addValues = (arr) => {
  let sum = arr.reduce((acc, number, idx) => {
    acc = acc + number;
    return acc;
  }, 0);
  return sum;
...
const addPurchases = (arr) => {
  let ammt = arr.reduce((acc, object, idx) => {
    acc = acc + object.purchasePrice;
    return acc;
  }, 0);
  return ammt;
};
...
const countNumberOfElements = (arr) => {
  let ammt = arr.reduce((acc, element) => {
    if (element){
      acc++;
      return acc;
    }
...
const reversedString = (str) => {
  let backwards = str.split('').reduce((acc, letter, idx) => {
    acc = letter + acc;
    return acc;
  }, '');
  return backwards;
