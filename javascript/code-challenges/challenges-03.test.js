...
const typeNum = (arr) => {
  const numArray = arr.filter(x => typeof(x) === 'number');
  return numArray;
};
...
const containsAnd = (arr) => {
  const andArray = arr.filter(x => x.includes('and'));
  return andArray;
};
...
const oddValues = (arr) => {
  const oddArray = arr.filter(x => x % 2 !== 0);
  return oddArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const notArray = arr.filter(x => !forbiddenValues.includes(x));
  return notArray;
};
